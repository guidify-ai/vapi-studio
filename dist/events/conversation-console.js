...
exports.snapshotUserMemory = snapshotUserMemory;
exports.isConversationConsoleEnabled = isConversationConsoleEnabled;
exports.printConversationConsole = printConversationConsole;
const daily_log_driver_1 = require("./daily-log.driver");
...
function formatValue(value, depth = 0) {
    if (value === null || value === undefined)
        return paint(ANSI.dim, String(value));
 ...
 */
function snapshotUserMemory(memory) {
...
function isConversationConsoleEnabled() {
...
const lastMemoryByScope = new Map();
function memoryScopeId(payload) {
...
function printMemoryDiff(memory, payload) {
    const snap = memory && typeof memory === 'object'
        ? snapshotUserMemory(memory)
        : {};
...
function intentionSummary(intentions) {
...
function listenSummary(listen) {
    ...
    const intents = (L.intentions ?? [])
        ...
        const boost = i.boost != null && i.boost !== 0
    ...
    const fields = (L.extract?.fields ?? [])
        ...
        .join(paint(ANSI.dim, ', '));
    const timeout = typeof L.timeoutSeconds === 'number'
 ...
 */
function printConversationConsole(level, type, payload) {
    const consoleOn = isConversationConsoleEnabled();
    lineSink = [];
    try {
        const turn = typeof payload.turnNumber === 'number'
            ? paint(ANSI.dim, `t${payload.turnNumber}`)
            : '';
        const call = typeof payload.providerCallId === 'string'
        ...
        switch (type) {
            ...
            case 'OPENING_TURN': {
                ...
                if (type === 'OPENING_TURN' && !userText.trim()) {
                    ...
                    line([
                        roleTag(ANSI.bgBlue, ANSI.white, 'USER'),
                        paint(ANSI.yellow, `(series of ${payload.parts.length})`),
                    ]);
                    for (const [i, part] of payload.parts.entries()) {
                        line([
                            paint(ANSI.dim, `       ${i + 1})`),
                ...
                if (payload.memory)
                    printMemoryDiff(payload.memory, payload);
                return;
            }
            ...
            case 'BRAIN_JUDGE_RESULT':
                line([
                    ...
                    Array.isArray(payload.reasoning)
                        ? paint(ANSI.dim, payload.reasoning.join(' · '))
                        : '',
                ]);
                return;
            ...
            case 'BRAIN_USAGE':
                line([
                    ...
                    paint(ANSI.dim, `in=${payload.promptTokens ?? 0} out=${payload.completionTokens ?? 0}`),
            ...
            case 'BRAIN_COST_SUMMARY': {
                ...
                line([
                    ...
                    paint(ANSI.yellow + ANSI.bold, String(payload.money ?? `$${payload.estimatedUsd ?? 0}`)),
                    paint(ANSI.dim, `tokens=${payload.totalTokens ?? 0} (in ${payload.promptTokens ?? 0} / out ${payload.completionTokens ?? 0})`),
                    paint(ANSI.dim, `calls=${payload.calls ?? 0}`),
                ]);
                if (payload.byModel && typeof payload.byModel === 'object') {
                    for (const [model, row] of Object.entries(payload.byModel)) {
                        line([
                            paint(ANSI.dim, '       '),
                            paint(ANSI.cyan, model),
                            paint(ANSI.dim, `×${row.calls ?? 0}`),
                            paint(ANSI.yellow, typeof row.estimatedUsd === 'number'
            ...
            case 'INTENTION_SCAN':
                ...
                if (payload.extracted && Object.keys(payload.extracted).length) {
                    line([
            ...
            case 'LISTEN_EXTRACTED':
                line([
                    ...
                    Array.isArray(payload.missingRequired) &&
                        payload.missingRequired.length
            ...
            case 'ROUTE_FAILED': {
                ...
                const rejected = Array.isArray(payload.rejected)
                    ...
                    : [];
                line([
                    roleTag(type === 'ROUTE_FAILED' ? ANSI.bgRed : ANSI.bgGreen, ANSI.white, type === 'ROUTE_FAILED' ? 'FAIL' : 'ROUTE'),
                    winner
                        ? paint(ANSI.green + ANSI.bold, `${winner.class ?? '?'} / ${winner.nodeId ?? '?'} ← ${winner.intention ?? ''}`)
            ...
            case 'INTENTION_MATCHED': {
                line([
                    roleTag(type === 'INTENTION_FORCE' ? ANSI.bgYellow : ANSI.bgCyan, ANSI.white, type === 'INTENTION_FORCE' ? 'IFORCE' : 'IMATCH'),
                                    ...
                                    : type === 'FORM_RESENT'
                                        ...
                                        : type === 'FORM_SUBMITTED'
                                            ...
                                            : type === 'FORM_DELIVER_TIMEOUT'
                                                ? 'deliver⏱'
                                                : 'fillout⏱';
                const bg = type.endsWith('TIMEOUT')
                    ? ANSI.bgRed
                    : type === 'FORM_SUBMITTED'
                        ? ANSI.bgGreen
            ...
            default:
                if (level === 'error' || level === 'warn') {
                    line([
                        roleTag(level === 'error' ? ANSI.bgRed : ANSI.bgYellow, ANSI.white, level.toUpperCase().padEnd(4, ' ')),
                        paint(level === 'error' ? ANSI.red : ANSI.yellow, type),
                        formatValue(payload),
                    ]);
                }
                else if ((0, studio_env_1.envFlag)('STUDIO_CONSOLE_DEBUG_ALL', undefined, false)) {
                    line([
        ...
        (0, daily_log_driver_1.appendDailyLog)(rows, payload);
    }
}
//# sourceMappingURL=conversation-console.js.map