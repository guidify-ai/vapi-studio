...
import type { FormExposeHandle, FormFieldSpec } from './form.types';
...
function inputType(field: FormFieldSpec): string {
  switch (field.type) {
    case 'email':
      return 'email';
    case 'tel':
...
function renderField(field: FormFieldSpec): string {
...
export function renderFormThanksHtml(options?: {
  title?: string;
  body?: string;
...
export function renderFormGoneHtml(message?: string): string {
  const body = escapeHtml(
    message ?? 'This form link is missing or has already been completed.',
  );
  return `<!DOCTYPE html>
