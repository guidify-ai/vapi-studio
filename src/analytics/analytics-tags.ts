 ...
 */
export function normalizeAnalyticsFunnels(
  funnels: string[] | undefined | null,
): string[] | undefined {
  if (!funnels?.length) return undefined;
  const clean = [
    ...new Set(
      funnels.map((f) => String(f ?? '').trim()).filter((f) => f.length > 0),
    ),
  ];
